...
public class Markov {
	...
	public static Markov fromString(String source) {
		...
		result.steady = Distribution.fromString(source);

		// first pass, over source string -> calculate frequencies
		int[][] transitionCount = new int[Distribution.RANGE][Distribution.RANGE];
		int[] totalTransitions = new int[Distribution.RANGE];
		...
		for (int i = 0; i < Distribution.RANGE; i++) {
			if (totalTransitions[i] > 0) {
				for (int j = 0; j < Distribution.RANGE; j++) {
					result.state[i].p[j] = (double) transitionCount[i][j] / totalTransitions[i];
				}
			}
		}
	...
	public double entropy() {
		...
		// Calculate entropy as weighted sum of state entropies
		for (int i = 0; i < Distribution.RANGE; i++) {
			if (steady.p[i] > 0) {
				...
				for (int j = 0; j < Distribution.RANGE; j++) {
					if (state[i].p[j] > 0) {
						stateEntropy -= state[i].p[j] * (Math.log(state[i].p[j]) / Math.log(2));
					}
				}

				sum += steady.p[i] * stateEntropy;
			}
		}
